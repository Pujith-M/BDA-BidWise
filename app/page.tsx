...
export default function Page() {
  ...
  const [registrationRate, setRegistrationRate] = useState('7')

  const result = useMemo(() => {
    const pricePerSqm = Math.max(0, Number(price) || 0)
    const areaSqm = Math.max(0, Number(area) || 0)
    const registration = Math.max(0, Number(registrationRate) || 0)
    ...
    return {
      ...
      total,
      upfront: total * 0.25,
      registration: total * registration / 100,
      totalWithCharges: total + total * registration / 100,
    }
  }, [price, area, registrationRate])

  const reset = () => { setPrice('50000'); setArea('100'); setRegistrationRate('7') }
        ...
        <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-[0_12px_40px_rgba(16,60,82,0.06)] sm:p-8">
            ...
            <div className="space-y-5">
              ...
              <div><label htmlFor="registration" className="mb-2 flex items-center justify-between text-sm font-semibold text-slate-700"><span>Registration & stamp duty</span><span className="font-normal text-slate-400">editable estimate</span></label><div className="relative"><input id="registration" type="number" min="0" step="0.1" value={registrationRate} onChange={e => setRegistrationRate(e.target.value)} className="h-14 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 pr-12 text-lg font-semibold outline-none transition focus:border-[#e68a4a] focus:ring-4 focus:ring-orange-100" /><span className="absolute right-4 top-1/2 -translate-y-1/2 font-semibold text-slate-400">%</span></div><p className="mt-2 text-xs text-slate-400">Adjust this to the latest rate applicable to your BDA property.</p></div>
            </div>
          ...
          <section className="rounded-3xl bg-[#103c52] p-6 text-white shadow-[0_16px_50px_rgba(16,60,82,0.2)] sm:p-8">
            ...
            <div className="grid gap-4 sm:grid-cols-2"><Metric label="25% upfront to BDA" value={formatINR(result.upfront)} emphasis /><Metric label="Registration charges" value={formatINR(result.registration)} /><Metric label="Area in square feet" value={`${formatNumber(result.areaSqft)} sq ft`} /><Metric label="Rate per square foot" value={`${formatINR(result.pricePerSqft)} / sq ft`} /></div>
            <div className="mt-8 rounded-2xl bg-[#1a5269] p-5"><div className="flex items-center justify-between gap-3"><div><p className="text-sm text-[#b7d2d9]">Total including registration</p><p className="mt-1 text-2xl font-bold">{formatINR(result.totalWithCharges)}</p></div><ArrowRight className="text-[#f0a467]" size={23} /></div></div>
          </section>
        </div>
