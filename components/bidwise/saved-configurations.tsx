  ...
  return <section className="mb-6" aria-labelledby="saved-configurations-title">
    <div className="mb-2 flex items-center justify-between gap-3 px-1">
      <div className="flex min-w-0 items-center gap-2">
        <h2 id="saved-configurations-title" className="text-xs font-bold uppercase tracking-[0.16em] text-[#103c52]">Site configurations</h2>
        {sharedName ? <span className="truncate text-xs text-slate-500">Shared: {sharedName}</span> : null}
      </div>
      <div className="flex shrink-0 items-center gap-1.5">
        <button type="button" onClick={onSave} aria-label="Save current site configuration" title="Save current configuration" className="flex size-9 items-center justify-center rounded-lg border border-slate-200 bg-white text-[#103c52] shadow-sm transition hover:border-[#bfe2d8] hover:bg-[#f1faf7] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-orange-100"><Save size={15} /></button>
